...
public class WordWriter extends OutputWriter {
    ...
    private final String FONT_FAMILY = "Verdana";
    private final int FONT_SIZE_DOCUMENT_HEADER = 6;
    private final int FONT_SIZE_DOCUMENT_FOOTER = 6;
    private final int FONT_SIZE_HEADLINE = 12;
    private final int FONT_SIZE_SUB_HEADLINE = 10;
    private final int FONT_SIZE_TEXT = 8;
    ...
    private void writeDocumentHeader() {
        ...
        run.setText(this.profile.getPerson().getCompany() + ", " + this.profile.getPerson().getSurname() + " " + this.profile.getPerson().getName());
     ...
     * @param run   Run Object
     * @param text  Headline
     * @param addCR Add carriage return
     */
    private void writeBoldHeader(XWPFRun run, String text, boolean addCR) {
        run.setFontSize(FONT_SIZE_TEXT);
        run.setFontFamily(FONT_FAMILY);
        run.setBold(true);
        run.setText(text);
     ...
     * Write project tasks
     *
     * @param tasks List of {@link String}
     */
    ...
    private void writeProjectHeader(Project project, XWPFRun run) {
        ...
        this.writeTabs(WordUtils.calcTabsForProjectSummary(this.resourceBundle.getString("duration")), run);
        Period duration = DateUtils.calculatePeriod(project.getStart(), project.getEnd());
        run.setText(DateUtils.format(project.getStart()) + " - " + DateUtils.format(project.getEnd()) + " (" + DateUtils.format(duration) + ")");
        run.addCarriageReturn();

        run.setText(this.resourceBundle.getString("industry"));
        this.writeTabs(WordUtils.calcTabsForProjectSummary(this.resourceBundle.getString("industry")), run);
        ...
        this.writeTabs(WordUtils.calcTabsForProjectSummary(this.resourceBundle.getString("involved_persons")), run);
    ...
    void writeTechnologyStack(List<Skill> skills) {
        Map<String, List<Skill>> groupedSkills = skills.stream().collect(Collectors.groupingBy(Skill::getCat));
        XWPFParagraph p = this.doc.getLastParagraph();
    ...
    void writeSkillCategory(Map.Entry<String, List<AggregatedSkillMapEntry>> category) {
        ...
        for (int i = 1; i <= category.getValue().size(); i++) {
            ...
            this.setContentCellForTable(i, 1, table, Integer.toString(category.getValue().get(i - 1).getProjectAmount()));
            this.setContentCellForTable(i, 2, table, DateUtils.format(category.getValue().get(i - 1).getPeriod()));
            this.setContentCellForTable(i, 3, table, String.join(", ", category.getValue().get(i - 1).getVersions()));
        }
    }
    ...
    void writeIndustrySkillMatrix() {
        ...
        for (Map.Entry<String, AggregatedIndustryMapEntry> entry : skills.entrySet()) {
            ...
            this.setContentCellForTable(counter, 1, table, DateUtils.format(entry.getValue().getPeriod()));
            this.setContentCellForTable(counter, 2, table, Integer.toString(entry.getValue().getProjectAmount()));
            counter++;
        }
    }
